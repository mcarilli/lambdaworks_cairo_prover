...
#[derive(Clone)]
pub struct CubicAIR<F>
where
    F: IsFFTField,
{
    context: AirContext,
    trace_length: usize,
    pub_inputs: CubicPublicInputs<F>,
}

#[derive(Clone, Debug)]
pub struct CubicPublicInputs<F>
where
    F: IsFFTField,
{
...
impl<F> AIR for CubicAIR<F>
where
    F: IsFFTField,
{
    type Field = F;
    type RAPChallenges = ();
    type PublicInputs = CubicPublicInputs<Self::Field>;
    ...
    fn compute_transition(
        ...
        vec![&second_row[0] - &first_row[0] * &first_row[0] * &first_row[0]]
    }
...
pub fn cubic_trace<F: IsFFTField>(
    ...
    for i in 1..(trace_length) {
        ret.push(ret[i - 1].clone() * ret[i - 1].clone() * ret[i - 1].clone());
    }
